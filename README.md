...
src/                         TypeScript engine (zero runtime deps)
  ...
  forecast.ts   forecast math (extremized GMO, quantiles, overlap-scaled k, golden-section + recalibration), analytics gate, ledger, calibration, scenario driver catalog + grounding gate + ledger-fitted sim weight
...
test/           end-to-end test with a scripted mock model (no API key needed)
  ...
  unit/*.test.js individual suites for tools, crawl, memory, pdftext, webtools, searchcore, citations, forecast math, scenario simulation
```
