...
| `swarm forecasts [watch]` | List the forecast ledger (open + resolved). `watch` re-checks each open forecast's update triggers. |
| `swarm resolve [set <id> <yes\|no\|value\|void>]` | Resolve due forecasts with a mini-agent against the resolution criteria (audit file per call), or set an outcome manually. |
...
- **Forecast data**: Manifold, Polymarket, and Kalshi odds keyless; Metaculus joins when a free API token is set (`metaculusApiKey` — their API requires one now). Time series from FRED (free `fredApiKey`), World Bank, and Yahoo Finance, plus GDELT news-volume counts (labeled as attention, not probability). Panel size, extremization k, and the coherence probe are configurable (`forecastPanelSize`, `forecastExtremizeK`, `forecastCoherenceProbe`) via CLI or the Settings UI.
