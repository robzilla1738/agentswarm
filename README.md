...
src/                         TypeScript engine (zero runtime deps)
  ...
  codeintel.ts  code-mode repo intelligence: deterministic recon → RepoProfile (build/test/lint detection), check-output parsing, the validated BuildPlan partition (partitionWaves) + repo symbol-map, the no-stub/dead-button diff scanner (scanStubs), and the commit-on-green / worktree / branch-safety git primitives
  ...
  session.ts    code-chat sessions: a durable multi-turn build conversation — persistent workspace, append-only turn index, prior-turn context block, cascade delete
...
ui/             Next.js 15 + Tailwind 4 web app (static-exported, served by the hub)
  ...
  app/code/page.tsx Code-chat home (new-chat + session list) and the per-session chat thread (CodeChat)
test/           end-to-end test with a scripted mock model (no API key needed)
  e2e.js        32 phases covering the full pipeline, including citations + force + resume + budget + verify + teams + forecast + scenario simulation + code mode (brownfield, best-of-N ensemble, engine repair, honest-unverified, ensemble fallback, greenfield) + code-chat sessions (managed workspace persisting across build → follow-up turns)
  unit/*.test.js individual suites for tools, crawl, memory, pdftext, webtools, searchcore, citations, forecast math, sports line anchoring + resolution, scenario simulation, and code mode (codeintel partition/repo-map + no-stub scan, codeledger, lock-key normalization, resume reducer, sessions)
```
